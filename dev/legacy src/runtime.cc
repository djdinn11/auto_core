...
void parse_and_set_action_map() {
    ...
    while (getline(config_file, line)) {
        ...
        size_t comma_delimter = line.find(',');
        size_t closing_brace = line.find('}');
        int key = get_numkey_vk_code(key_string);
        string primary = line.substr(opening_brace + 1, comma_delimter - opening_brace - 1);
        string secondary = line.substr(comma_delimter + 2, closing_brace - comma_delimter - 2);
